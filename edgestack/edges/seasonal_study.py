"""Preholdout evaluator for the preregistered Halloween seasonality family.

Implements configs/seasonal-study-v1.yaml exactly: 9 real trials (9 index and
sector ETFs x 1 calendar signal), long November through April versus
zero-return cash, positions changed ONLY at month-end closes (final October
close in, final April close out), costs charged per flip. The forward holdout
...
# Long season: the position held DURING November..April. The decision at a
# month-end close is therefore "is the NEXT month inside the season".
_SEASON_MONTHS = frozenset({11, 12, 1, 2, 3, 4})


def _load_config(path: Path) -> dict[str, Any]:
    payload = yaml.safe_load(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("seasonal study configuration must be a mapping")
    family = cast(Mapping[str, Any], payload["declared_family"])
    if int(family["real_trial_count"]) != 9:
        raise ValueError("declared trial count does not match the preregistration")
    return cast(dict[str, Any], payload)


def _monthly_signal(daily_adjusted: pd.Series) -> pd.Series:
    """Boolean long/flat decision at each month-end close (calendar-causal)."""

    month_end = daily_adjusted.groupby(
        pd.PeriodIndex(daily_adjusted.index, freq="M")
    ).tail(1)
    next_month = (pd.DatetimeIndex(month_end.index).month % 12) + 1
    decided = pd.Series(
        [month in _SEASON_MONTHS for month in next_month], index=month_end.index
    )
    return decided.astype(bool)
...
def build_streams(
    ...
    for instrument in instruments:
        ...
        decided = _monthly_signal(series)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--config", default="configs/seasonal-study-v1.yaml")
